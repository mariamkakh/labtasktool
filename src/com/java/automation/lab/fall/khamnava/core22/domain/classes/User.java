...
public class User extends AbstractUser {
    private UserAction[] action = {UserAction.ADD_TASK, UserAction.ADD_COMMENT, UserAction.ADD_ATTACHMENT,
            UserAction.MOVE_TASK, UserAction.ADD_TASK_TO_TRASH, UserAction.EDIT_TASK, UserAction.WATCH_SPRINT,
            UserAction.WATCH_TASK, UserAction.ASSIGN_TASK};

    public User(Info significantInfo, UserRole role, VisualBoard[] visualBoard) {
    ...
    public boolean equals(Object that) {
        ...
        return getSignificantInfo().equals(((User) that).getSignificantInfo()) &&
                getRole().equals(((User) that).getRole()) &&
                getVisualBoard().equals(((User) that).getVisualBoard()) &&
                Arrays.equals(action, ((User) that).action);
    }
